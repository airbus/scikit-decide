...
def test_reynolds_number():
    ...
    mach_number = 0.78
    pressure_0 = 100_000
    assert atm_params.reynolds_number(
        wing_span, wing_aspect_ratio, mach_number, T_15, pressure_0
    ) == pytest.approx(64_412_748, rel=1e-3)
...
def test_rocd():
    ...
    delta_time = 30
    assert jet.rate_of_climb_descent(
        altitude_current, altitude_next, delta_time
    ) == pytest.approx((altitude_next - altitude_current) / (delta_time / 60), rel=1e-4)
...
def test_lift_coefficient():
    ...
    pressure_0 = 100_000
    climb_angle = 0
    assert atm_params.lift_coefficient(
        wing_surface_area, mtow, pressure_0, T_0, mach_number, climb_angle
    ) == pytest.approx(0.13828, rel=1e-3)
...
def test_skin_friction_coefficient():
    ...
    mach_number = 0.78
    pressure_0 = 100_000
    reynolds_number = atm_params.reynolds_number(
        wing_span, wing_aspect_ratio, mach_number, T_15, pressure_0
    )
    assert atm_params.skin_friction_coefficient(reynolds_number) == pytest.approx(
        0.0269 / (reynolds_number**0.14), rel=1e-5
    )
...
def test_zero_lift_drag():
    ...
    assert atm_params.zero_lift_drag_coefficient(
        0.0269 / (reynolds_number**0.14), psi_0
    ) == pytest.approx(0.0269 / (reynolds_number**0.14) * psi_0, rel=1e-5)
...
def test_oswald_efficiency_factor2():
    ...
    assert atm_params.oswald_efficiency_factor(
        zero_lift_drag, aircraft_parameters
    ) == pytest.approx(0.85, rel=1e-2)
...
def test_k1():
    ...
    assert atm_params._non_vortex_lift_dependent_drag_factor(
        zero_lift_drag, cos_sweep
    ) == pytest.approx(0.80 * (1 - 0.53 * cos_sweep) * zero_lift_drag, rel=1e-2)
...
def test_wave_drag_coefficient():
    ...
    c_lift = 0.13828
    mach_number = 0.78
    assert atm_params.wave_drag_coefficient(
        mach_number, c_lift, aircraft_parameters
    ) == pytest.approx(0.00049245, rel=1e-2)
...
def test_airframe_drag_coefficient():
    ...
    psi_0 = aircraft_parameters["psi_0"]
    reynolds_number = 64_412_748
    zero_lift_drag = atm_params.zero_lift_drag_coefficient(
        0.0269 / (reynolds_number**0.14), psi_0
    )
    wave_drag = 0.00049245
    oswald_efficiency = atm_params.oswald_efficiency_factor(
        zero_lift_drag, aircraft_parameters
    )
    wing_aspect_ratio = aircraft_parameters["wing_aspect_ratio"]
    assert atm_params.airframe_drag_coefficient(
        zero_lift_drag, wave_drag, c_lift, oswald_efficiency, wing_aspect_ratio
    ) == pytest.approx(0.019465667160460293, rel=1e-2)
...
def test_thrust_force():
    ...
    mtow = aircraft_parameters["amass_mtow"]
    climb_angle = 0
    assert atm_params.thrust_force(
        mtow, c_lift, c_drag, 0, climb_angle
    ) == pytest.approx(101470, rel=1e-2)
...
def test_engine_thrust_coefficient():
    ...
    mach_number = 0.78
    pressure_0 = 100_000
    assert atm_params.engine_thrust_coefficient(
        f_thrust, mach_number, pressure_0, wing_surface_area
    ) == pytest.approx(0.01946566716046029, rel=1e-1)
...
def test_thrust_coefficient_at_max_efficiency():
    ...
    aircraft_parameters = load_aircraft_engine_params("A320")
    mach_number = 0.78
    assert atm_params.thrust_coefficient_at_max_efficiency(
        mach_number, aircraft_parameters["m_des"], aircraft_parameters["c_t_des"]
    ) == pytest.approx(0.03267886563990706, rel=1e-1)
...
def test_max_available_thrust_coeff():
    ...
    T_15 = 288.15
    mach_number = 0.78
    assert op_lim.max_available_thrust_coefficient(
        T_15, mach_number, c_t_eta_b, aircraft_parameters
    ) == pytest.approx(0.019882296940110962, rel=1e-1)
...
def test_overall_prop_efficiency():
    ...
    aircraft_parameters = load_aircraft_engine_params("A320")
    mach_number = 0.78
    assert atm_params.overall_propulsion_efficiency(
        mach_number, c_t, c_t_eta_b, aircraft_parameters, eta_over_eta_b_min
    ) == pytest.approx(0.28091506251406534, rel=1e-2)
...
def test_fuel_mass_flow_rate():
    ...
    mach_number = 0.78
    pressure_0 = 100_000
    assert atm_params.fuel_mass_flow_rate(
        pressure_0, T_15, mach_number, c_t, engine_efficiency, wing_surface_area, q_fuel
    ) == pytest.approx(702.9593810712935, rel=1e-1)
...
def test_fuel_flow_correction(rocd, expected):
    ...
    mach_number = 0.78
    pressure_0 = 100_000
    assert atm_params.fuel_flow_correction(
        fuel_flow,
        altitude_current,
        T_15,
        pressure_0,
        mach_number,
        aircraft_parameters["ff_idle_sls"],
        aircraft_parameters["ff_max_sls"],
        flight_phase,
