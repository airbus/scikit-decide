...
class StableBaseline(Solver, Policies, Restorable):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        use_action_masking: if True, the domain will be wrapped in a gymnasium environment exposing `action_masks()`.
    ...
    def _sample_action(
        ...
        action, _ = self._algo.predict(self._unwrap_obs(observation), **kwargs)
...
def as_masked_gymnasium_env(domain: Domain) -> gym.Env:
    ...
    Uses `domain.action_masks()` when existing, else tries to derive one by using `domain.is_applicable_action()`
    provided that `domain.get_action_space()` is a `skdecide.core.EnumerableSpace`.

    For computational efficiency, it is generally better to have properly implemented `domain.action_masks()`.

    """
    env = AsGymnasiumEnv(domain=domain)
    if hasattr(domain, "action_masks"):
        action_masks_fn = lambda env: env.domain.action_masks()
    else:
        action_masks_fn = lambda env: np.array(
            [
                env.domain.is_applicable_action(action)
                for action in domain.get_action_space().get_elements()
            ]
        )
    return ActionMasker(
        env=env,
        action_mask_fn=action_masks_fn,
    )
