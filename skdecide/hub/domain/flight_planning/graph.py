...
def calculate_grid_point_coords(
    ...
    point_on_path = p0.destination(
        distance=x_local_km * 1000, bearing=initial_path_bearing
    )
    ...
    if y_local_km != 0:
        ...
        grid_point = point_on_path.destination(
            distance=abs(y_local_km) * 1000, bearing=lateral_bearing
        )
    else:
        grid_point = (
            point_on_path  # No lateral offset, point is directly on the main path
        )
...
def create_flight_graph(
    ...
    # --- Setup ---
    if graph_width == "medium":
        y_grid_width_km = 1000.0
    elif graph_width == "wide":
        y_grid_width_km = 2000.0
    else:
        y_grid_width_km = 500.0
    total_x_distance_km = p0.distanceTo(p1) / 1000.0
    if not flight_levels_ft:
        raise ValueError("flight_levels_ft cannot be empty.")
    flight_levels_ft.sort()
    x_step_km = (
        total_x_distance_km / (nb_forward_points - 1)
        if nb_forward_points > 1
        else total_x_distance_km
    )
    y_step_km = (
        y_grid_width_km / (nb_lateral_points - 1)
        if nb_lateral_points > 1
        else y_grid_width_km
    )
    G = nx.DiGraph()
    ...
    if nb_descent_steps > 0 and end_alt_ft < min(flight_levels_ft):
        ...
        for step in range(1, nb_descent_steps + 1):
            descent_altitudes.add(
                round(min(flight_levels_ft) - (step * descent_alt_step_val), 0)
            )
    start_end_altitudes = {round(start_alt_ft, 0), round(end_alt_ft, 0)}
    all_possible_altitudes = (
        climb_altitudes | cruise_altitudes | descent_altitudes | start_end_altitudes
    )
    ...
    G.add_node(
        start_node_key,
        latlon=p0,
        flight_level=round(start_alt_ft, 0) * ft,
        phase="start",
    )
    ...
    if need_climb:
        ...
        for i in range(1, climb_phase_end_x_idx + 1):
            ...
            step_idx = min(
                int(climb_step_ratio * len(climb_altitudes_sorted)),
                len(climb_altitudes_sorted) - 1,
            )
            target_alt_ft = climb_altitudes_sorted[step_idx]
            current_z_idx_for_climb = altitude_to_z_idx[target_alt_ft]
            num_active_lateral_points = 1 + int(
                (nb_lateral_points - 1) * (i / climb_phase_end_x_idx)
            )
            active_y_indices = []
            if num_active_lateral_points == 1:
                active_y_indices.append(y_center_idx)
            else:
                start_y_idx_for_active = y_center_idx - (num_active_lateral_points // 2)
                for k in range(num_active_lateral_points):
                    active_y_indices.append(start_y_idx_for_active + k)
            for j in active_y_indices:
                y_local_km = -y_grid_width_km / 2 + j * y_step_km
                node_key = (i, j, current_z_idx_for_climb)
                lat, lon, _ = calculate_grid_point_coords(
                    p0, p1, x_local_km, y_local_km, target_alt_ft
                )
                G.add_node(
                    node_key,
                    latlon=LatLon(lat, lon, height=target_alt_ft * 0.3048),
                    flight_level=target_alt_ft,
                    phase="climb",
                )
    ...
    for i in range(climb_phase_end_x_idx, descent_phase_start_x_idx + 1):
        ...
        for j in range(nb_lateral_points):
            ...
            for fl_ft in cruise_altitudes_sorted:
                if (
                    i == climb_phase_end_x_idx
                    and need_climb
                    and fl_ft != first_cruise_ft
                ) or (
                    i == descent_phase_start_x_idx
                    and need_descent
                    and fl_ft != first_cruise_ft
                ):
                    continue
                ...
                lat, lon, _ = calculate_grid_point_coords(
                    p0, p1, x_local_km, y_local_km, fl_ft
                )
                G.add_node(
                    node_key,
                    latlon=LatLon(lat, lon, height=fl_ft * 0.3048),
                    flight_level=fl_ft,
                    phase="cruise",
                )

    if need_descent:
        descent_alt_step_val = (min(flight_levels_ft) - end_alt_ft) / max(
            nb_descent_steps, 1
        )
        for i in range(descent_phase_start_x_idx + 1, nb_forward_points - 1):
            x_local_km = i * x_step_km
            descent_step_progress = i - descent_phase_start_x_idx
            target_alt_ft_raw = min(flight_levels_ft) - (
                descent_step_progress * descent_alt_step_val
            )
            target_alt_ft = round(max(end_alt_ft, target_alt_ft_raw), 0)
            if target_alt_ft not in altitude_to_z_idx:
                continue
            current_z_idx_for_descent = altitude_to_z_idx[target_alt_ft]
            steps_remaining_to_end = (nb_forward_points - 1) - i
            lateral_compression_factor = (
                (steps_remaining_to_end / nb_descent_steps)
                if nb_descent_steps > 0
                else 0
            )
            num_active_lateral_points = 1 + int(
                (nb_lateral_points - 1) * lateral_compression_factor
            )
            active_y_indices = []
            if num_active_lateral_points == 1:
                active_y_indices.append(y_center_idx)
    ...
    G.add_node(
        end_node_key, latlon=p1, flight_level=round(end_alt_ft, 0) * ft, phase="end"
    )
        ...
        for j in range(nb_lateral_points):
            for k in range(max_z_idx + 1):
                ...
                if need_climb and i < climb_phase_end_x_idx:
                    ...
                    step_idx_next = min(
                        int(climb_step_ratio_next * len(climb_altitudes_sorted)),
                        len(climb_altitudes_sorted) - 1,
                    )
                    ...
                    if current_alt_ft < first_cruise_ft:
                ...
                if climb_phase_end_x_idx <= i <= descent_phase_start_x_idx:
                    if current_alt_ft in flight_levels_ft:
                        ...
                        if (
                            k + 1 <= max_z_idx
                            and z_idx_to_altitude[k + 1] in flight_levels_ft
                        ):
                            allowed_dz_relative.add(1)
                        if k - 1 >= 0 and z_idx_to_altitude[k - 1] in flight_levels_ft:
                            allowed_dz_relative.add(-1)
                ...
                if i > descent_phase_start_x_idx and need_descent:
                    ...
                    target_alt = round(
                        min(flight_levels_ft)
                        - descent_step_progress_next * descent_alt_step_val,
                        0,
                    )
