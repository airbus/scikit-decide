...
class NodeLabel(Enum):
    PREDICATE = "predicate"  # predicate/fluent type node
    ...
    NUMERIC = "numeric"  # numeric + predicate = fluent
    NEGATED = "negated"  # negation of an atom
    EFFECT_TARGET = "effect_target"  # flag for fluent updated by a numeric effect
...
class LLGEncoder:
    ...
    def _compute_gym_graph(self) -> gym.spaces.GraphInstance:
        ...
        nodes[self._function_nodes, map_nodelabel2int[NodeLabel.PREDICATE]] = 1
        nodes[self._function_nodes, map_nodelabel2int[NodeLabel.NUMERIC]] = 1
        nodes[self._effect_target_nodes, map_nodelabel2int[NodeLabel.EFFECT_TARGET]] = 1
    ...
    def _encode_action(self, action: Action) -> None:
        ...
        for e in action.effect.effects:
            ...
            if isinstance(atomic_effect, (AddEffect, DelEffect)):
                ...
                scheme_pred_node = self._encode_fluent(
                    fluent=fluent,
                    edge_label=edge_label,
                    grounded=False,
                    variable_nodes=variable_nodes,
                ...
                self._effect_target_nodes.append(scheme_pred_node)
                self._encode_numeric_expression(
    ...
    def _encode_numeric_expression(
        ...
        if isinstance(expr, BinaryOperation):
            ...
            if isinstance(expr, Addition):
                self._map_node2operator[expr_node] = NodeLabel.PLUS
            elif isinstance(expr, Subtraction):
                self._map_node2operator[expr_node] = NodeLabel.MINUS
            elif isinstance(expr, Division):
                self._map_node2operator[expr_node] = NodeLabel.DIVIDE
            elif isinstance(expr, Multiplication):
                self._map_node2operator[expr_node] = NodeLabel.MULTIPLY
            else:
                raise NotImplementedError()
            self._add_edge(expr_node, lhs_node, label=edge_label, subgraph=subgraph)
            self._add_edge(expr_node, rhs_node, label=edge_label, subgraph=subgraph)
    ...
    def _prepare_for_plot(
        ...
        ind = (
            ...
            & np.logical_not(
                graph.nodes[:, map_nodelabel2int[NodeLabel.PREDICATE]].astype(bool)
            )
        ).nonzero()[0]
        node_labels_np[ind] = [
...
def decode_llg(
    ...
    predicate_nodes = (
        ...
        * (1 - graph.nodes[:, map_nodelabel2int[NodeLabel.NUMERIC]])  # not numeric
    ).nonzero()[0]
    function_nodes = (
        graph.nodes[:, map_nodelabel2int[NodeLabel.PREDICATE]]  # predicate
        * graph.nodes[:, map_nodelabel2int[NodeLabel.NUMERIC]]  # numeric
    ).nonzero()[0]
