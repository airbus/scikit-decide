...
from airlaps.hub.domain.gym import GymPlanningDomain, GymWidthDomain, \
    GymDiscreteActionDomain, GymDomainStateProxy, GymDomain
...
class GymUCTSampleDomain(D):
    ...
    def reset(self):
        self.current_outcome = EnvironmentOutcome(observation=GymDomainStateProxy(state=super().reset(), context=self._gym_env.get_state()),
                                                  value=None,
                                                  termination=False,
                                                  info=None)
        return self.current_outcome.observation
    ...
    def get_next_state_distribution(self, state, action):
        ...
        outcome = self.step(action)
        observation = GymDomainStateProxy(state=outcome.observation._state, context=self._gym_env.get_state())
        self.current_outcome = EnvironmentOutcome(observation=observation, value=outcome.value, termination=outcome.termination, info=outcome.info)
        return DiscreteDistribution([(observation, 1.0)])
    
    def step(self, action):
        outcome = super().step(action)
        observation = GymDomainStateProxy(state=outcome.observation, context=self._gym_env.get_state())
        return EnvironmentOutcome(observation=observation, value=outcome.value, termination=outcome.termination, info=outcome.info)
...
domain_factory = lambda: GymUCTSampleDomain(gym_env=gym.make(ENV_NAME),
                                            discretization_factor=5,
                                            max_depth=50)

if True:#UCT.check_domain(domain_factory()):
    solver_factory = lambda: UCT(time_budget = 5000,
                                 ...
                                 parallel=False,
                                 debug_logs = False)
    solver = GymUCTSampleDomain.solve_with(solver_factory, domain_factory)
