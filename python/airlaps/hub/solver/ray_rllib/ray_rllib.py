...
class RayRLlib(Solver, Policies, Restorable):
    ...
    def _init_algo(self, domain_factory: Callable[[], D]):
        ...
        # Instanciate algo
        register_env('airlaps_env', lambda _: AsRLlibMultiAgentEnv(domain_factory()))
        self._algo = self._algo_class(env='airlaps_env', config=self._config)
...
class AsRLlibMultiAgentEnv(MultiAgentEnv):
    ...
    def __init__(self, domain: D) -> None:
        ...
        # Parameters
        domain: The AIRLAPS domain to wrap as a RLlib multi-agent environment.
        """
        self._domain = domain
...
if __name__ == '__main__':
    ...
    from airlaps.hub.domain.rock_paper_scissors import RockPaperScissors
