...
struct PythonDomainProxyBase<Texecution>::PyObj<Derived,
                                                Tpyobj>::Implementation {

  typedef typename PythonDomainProxyBase<Texecution>::template PyObj<Derived,
                                                                     Tpyobj>
      PyObj;
  ...
  template <typename TTpyobj = Tpyobj,
            std::enable_if_t<
                !std::is_same<TTpyobj, py::object>::value &&
                    !std::is_base_of<py::object, TTpyobj>::value &&
                    std::is_base_of<py::detail::pyobject_tag, TTpyobj>::value &&
                    !std::is_default_constructible<TTpyobj>::value,
                int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po) {}
  ...
  template <typename TTpyobj = Tpyobj,
            std::enable_if_t<std::is_same<TTpyobj, py::object>::value, int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po,
                        std::unique_ptr<py::object> &&o, bool check = true) {
    typename GilControl<Texecution>::Acquire
        acquire; // !(*po) class python if *po is of type py::bool_
    po = std::move(o);
    if (check && (!po || !(*po))) {
  ...
  static void construct(std::unique_ptr<Tpyobj> &po,
    ...
    if (check && (!o || !(*o) || !py::isinstance<Tpyobj>(*o))) {
      throw std::runtime_error(
          std::string("Python ") + Derived::class_name +
          " object not initialized as a " +
  ...
  template <typename TTpyobj = Tpyobj,
            std::enable_if_t<!std::is_same<TTpyobj, py::object>::value &&
                             ...
                             int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po,
                        std::unique_ptr<TTpyobj> &&o, bool check = true) {
    typename GilControl<Texecution>::Acquire
        acquire; // !(*po) class python if *po is of type py::bool_
    po = std::move(o);
    if (check && (!po || !(*po))) {
  ...
  template <typename TTpyobj = Tpyobj,
            std::enable_if_t<std::is_same<TTpyobj, py::object>::value, int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po, const py::object &o,
                        bool check = true) {
    typename GilControl<Texecution>::Acquire acquire;
  ...
  template <typename TTpyobj,
            std::enable_if_t<!std::is_same<TTpyobj, py::object>::value &&
                                 std::is_base_of<py::object, TTpyobj>::value,
                             int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po, const TTpyobj &o,
                        bool check = true) {
    typename GilControl<Texecution>::Acquire acquire;
  ...
  template <typename TTpyobj,
            std::enable_if_t<!std::is_base_of<py::object, TTpyobj>::value &&
                                 !std::is_base_of<PyObj, TTpyobj>::value,
                             int> = 0>
  static void construct(std::unique_ptr<Tpyobj> &po, const TTpyobj &o,
...
#define SK_PYOBJ_TEMPLATE_DECL                                                 \
  template <typename Texecution> template <typename Derived, typename Tpyobj>
...
SK_PY_OBJ_CLASS::PyObj() { Implementation::construct(_pyobj); }
...
SK_PYOBJ_TEMPLATE_DECL
const Tpyobj &SK_PY_OBJ_CLASS::pyobj() const { return *_pyobj; }
...
SK_PYOBJ_TEMPLATE_DECL
std::size_t
SK_PY_OBJ_CLASS::Hash::operator()(const PyObj<Derived, Tpyobj> &o) const {
  try {
...
SK_PYOBJ_TEMPLATE_DECL
bool SK_PY_OBJ_CLASS::Equal::operator()(
    const PyObj<Derived, Tpyobj> &o1, const PyObj<Derived, Tpyobj> &o2) const {
  try {
...
struct PythonDomainProxyBase<Texecution>::PyIter<T, Titerator>::Implementation {
  template <
      typename TTiterator = Titerator,
      std::enable_if_t<
          !std::is_same<TTiterator, py::detail::dict_iterator>::value, int> = 0>
  ...
  static std::unique_ptr<T> dereference_pointer(Titerator &pit) {
    ...
    return std::make_unique<T>(
        py::make_tuple(py::reinterpret_borrow<py::object>(pit->first),
                       py::reinterpret_borrow<py::object>(pit->second)));
  }
};

#define SK_PY_ITER_TEMPLATE_DECL                                               \
  template <typename Texecution> template <typename T, typename Titerator>

#define SK_PY_ITER_CLASS PythonDomainProxyBase<Texecution>::PyIter<T, Titerator>
...
SK_PY_ITER_TEMPLATE_DECL
SK_PY_ITER_TYPE &
SK_PY_ITER_CLASS::operator=(const PyIter<T, Titerator> &other) {
