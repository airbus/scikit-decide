...
struct PythonDomainProxy<Texecution, Tagent, Tobservability, Tcontrollability,
                         Tmemory>::
  ...
  std::unique_ptr<py::object>
  call([[maybe_unused]] const std::size_t *thread_id, const Tfunction &func,
       const Types &...args) {
    try {
...
struct PythonDomainProxy<Texecution, Tagent, Tobservability, Tcontrollability,
  ...
  std::unique_ptr<py::object> do_launch(const std::size_t *thread_id,
    ...
    nng::socket *conn = nullptr;
    {
      ...
      try {
        if (thread_id) {
          id = std::make_unique<py::object>(
              func(*_domain, args..., py::int_(*thread_id)));
        } else {
          id =
              std::make_unique<py::object>(func(*_domain, args..., py::none()));
        }
    ...
    if (conn) { // positive id returned (parallel execution, waiting for python
      ...
      try {
        ...
        if (msg.body().size() != 1 ||
            msg.body().data<char>()[0] != '0') { // error
          typename GilControl<Texecution>::Acquire acquire;
          id.reset();
          std::string pyerr(msg.body().data<char>(), msg.body().size());
          throw std::runtime_error(
              "SKDECIDE exception: C++ parallel domain received an exception "
              "from Python parallel domain: " +
              pyerr);
        }
      } catch (const nng::exception &e) {
        std::string err_msg("SKDECIDE exception when waiting for a response "
                            "from the python parallel domain: ");
      ...
      std::string err_msg(
          "Unable to establish a connection with the Python parallel domain");
      Logger::error(err_msg);
  ...
  std::unique_ptr<py::object> launch(const std::size_t *thread_id,
                                     const char *name, const Types &...args) {
    return do_launch(
        thread_id,
        [&name](py::object &d, auto... aargs) {
...
PythonDomainProxy<Texecution, Tagent, Tobservability, Tcontrollability,
                  Tmemory>::call(const std::size_t *thread_id,
  ...
  try {
    ...
    Logger::error(
        std::string(
            "SKDECIDE exception when calling anonymous domain method: ") +
        std::string(e.what()));
    throw;
  }
}
