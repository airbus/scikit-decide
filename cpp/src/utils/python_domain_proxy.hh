...
struct PartiallyObservable {
  PartiallyObservable() {}
};
struct FullyObservable {
  FullyObservable() {}
};

struct PartiallyControllable {
  PartiallyControllable() {}
};
struct FullyControllable {
  FullyControllable() {}
};
...
template <typename Texecution, typename Tagent = SingleAgent,
...
public:
  ...
  using PyObj =
      typename PythonDomainProxyBase<Texecution>::template PyObj<Derived,
                                                                 Tpyobj>;
  ...
  class AgentDataAccess<DData, TTagent,
                        typename std::enable_if<
                            std::is_same<TTagent, SingleAgent>::value>::type> {
  public:
    typedef DData Data;
    typedef void Agent;
  ...
  class AgentDataAccess<
      ...
      : public PyObj<DData, py::dict> {
  public:
    // AgentDataAccess inherits from pyObj<> to manage its python object but
    // DData is passed to PyObj as template parameter to print DData::class_name
    // when managing AgentDataAccess objects
    ...
    typedef typename PythonDomainProxy<Texecution, Tagent, Tobservability,
                                       Tcontrollability, Tmemory>::Agent Agent;
    ...
    class AgentDataAccessor
        : public PyObj<AgentDataAccessor, py::detail::item_accessor>,
          public AgentData {
    public:
  ...
  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::State>::Data State;
  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::Observation>::Data
      _Observation;

  typedef typename std::conditional<
      std::is_same<Tobservability, FullyObservable>::value, State,
      typename std::conditional<
          std::is_same<Tobservability, PartiallyObservable>::value,
          _Observation, void>::type>::type Observation;

  class MemoryState : public PyObj<MemoryState, py::list> {
  public:
    typedef typename PythonDomainProxy<Texecution, Tagent, Tobservability,
                                       Tcontrollability, Tmemory>::State State;
  ...
  typedef typename std::conditional<
      std::is_same<Tmemory, Markovian>::value, State,
      typename std::conditional<std::is_same<Tmemory, History>::value,
                                MemoryState, void>::type>::type Memory;

  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::Action, Tagent>::Data Action;
  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::Event>::Data _Event;

  typedef typename std::conditional<
      std::is_same<Tcontrollability, FullyControllable>::value, Action,
      typename std::conditional<
          std::is_same<Tcontrollability, PartiallyControllable>::value, _Event,
          void>::type>::type Event;

  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::ApplicableActionSpace>::Data
      ApplicableActionSpace;
  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::Value>::Data Value;
  typedef typename AgentDataAccess<
      typename PythonDomainProxyBase<Texecution>::Predicate>::Data Predicate;
  ...
  public:
    ...
    typedef typename PythonDomainProxy<Texecution, Tagent, Tobservability,
                                       Tcontrollability, Tmemory>::Value Value;
    typedef typename PythonDomainProxy<Texecution, Tagent, Tobservability,
                                       Tcontrollability, Tmemory>::Predicate
        Predicate;
    typedef typename AgentDataAccess<
        typename PythonDomainProxyBase<Texecution>::OutcomeInfo>::Data Info;
  ...
  public:
    ...
    static constexpr char situation_name[] =
        "state"; // mandatory since State == Observation in fully observable
                 // domains
  ...
  public:
    ...
    static constexpr char situation_name[] =
        "observation"; // mandatory since State == Observation in fully
                       // observable domains
  ...
  public:
    ...
    class NextStateDistributionValues
        : public PyObj<NextStateDistributionValues> {
    public:
      typedef typename PythonDomainProxyBase<Texecution>::template PyIter<
          DistributionValue>
          PyIter;
      ...
      NextStateDistributionValues &
      operator=(const NextStateDistributionValues &other);
  ...
  ApplicableActionSpace
  get_applicable_actions(const Memory &m,
                         const std::size_t *thread_id = nullptr);

  template <typename TTagent = Tagent, typename TTaction = Action,
            typename TagentApplicableActions = typename PythonDomainProxyBase<
                Texecution>::ApplicableActionSpace>
  ...
  EnvironmentOutcome step(const Event &e,
                          const std::size_t *thread_id = nullptr);
  EnvironmentOutcome sample(const Memory &m, const Event &e,
                            const std::size_t *thread_id = nullptr);
  State get_next_state(const Memory &m, const Event &e,
                       const std::size_t *thread_id = nullptr);
  NextStateDistribution
  get_next_state_distribution(const Memory &m, const Event &e,
                              const std::size_t *thread_id = nullptr);
  ...
  std::unique_ptr<py::object> call(const std::size_t *thread_id,
                                   const Tfunction &func, const Types &...args);
