...
struct SequentialExecution {
  static constexpr std::execution::sequenced_policy policy =
      std::execution::seq;
  ...
  inline void protect(const std::function<void()> &f,
                      [[maybe_unused]] Tmutex &m) {
    f();
  }

  inline void protect(const std::function<void()> &f) { f(); }

  inline static std::string print_type() { return class_name; }

  inline static std::string print_thread() { return std::string(); }
...
struct ParallelExecution {
  ...
  inline void protect(const std::function<void()> &f) { protect(f, _mutex); }
...
struct ParallelExecution {
  ...
  struct RecursiveMutex {
    RecursiveMutex() { omp_init_nest_lock(&_lock); }

    ~RecursiveMutex() { omp_destroy_nest_lock(&_lock); }
...
struct ParallelExecution {
  ...
  static constexpr char class_name[] =
      "sequential (no parallelization support - compile with c++-17 "
      "parallelization feature or with openmp)";
...
void for_each(int policy, ForwardIt first, ForwardIt last, UnaryFunction2 f) {
  // openMP handles random access iterators from version 3.0 only (thus not all
  // container iterators)
...
#pragma omp parallel for
  for (int i = 0; i < (int)v.size();
       i++) { // MSVC requires signed integers for openmp loops
    f(*v[i]);
  }
}
