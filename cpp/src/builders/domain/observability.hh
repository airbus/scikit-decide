...
class PartiallyObservableDomain {
  static_assert(std::is_same<typename TstateSpace::element_type, Tstate>::value,
                "State space elements must be of type Tstate");
  static_assert(
      std::is_base_of<Space<Tstate>, TstateSpace>::value,
      "State space type must be derived from skdecide::Space<Tstate>");
  ...
  static_assert(
      std::is_same<typename TobservationDistribution::element_type,
                   Tobservation>::value,
      "Observation distribution elements must be of type Tobservation");
  static_assert(
      std::is_base_of<Distribution<Tobservation>,
                      TobservationDistribution>::value,
      "State space type must be derived from skdecide::Space<Tstate>");
...
public:
  ...
  get_observation_distribution(const State &state, const Event &event) = 0;
...
class FullyObservableDomain
    : public virtual PartiallyObservableDomain<
          Tstate, Tstate, Tevent, TstateSpace, TstateSpace,
...
public:
  ...
  inline virtual ObservationDistributionPtr
  get_observation_distribution(const State &state, const Event &event) {
