...
public:
  ...
  typedef typename SequenceContainer<Derived, Variable>::SymbolVector
      VariableVector;
  ...
  template <typename T> const VariablePtr &append_variable(const T &variable) {
    return SequenceContainer<Derived, Variable>::append(variable);
  }

  template <typename T> void remove_variable(const T &variable) {
    SequenceContainer<Derived, Variable>::remove(variable);
  }

  template <typename T> VariableVector get_variable(const T &variable) const {
    return SequenceContainer<Derived, Variable>::get(variable);
  }

  const VariablePtr &variable_at(const std::size_t &index) const;
...
#include "impl/variable_container_impl.hh"
#endif
