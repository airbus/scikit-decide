...
Requirements::Requirements()
    : _equality(false), _strips(false), _typing(false),
      _negative_preconditions(false), _disjunctive_preconditions(false),
      _existential_preconditions(false), _universal_preconditions(false),
      _conditional_effects(false), _numeric_fluents(false),
      _object_fluents(false), _durative_actions(false), _time(false),
      _action_costs(false), _modules(false), _duration_inequalities(false),
      _continuous_effects(false), _derived_predicates(false),
      _timed_initial_literals(false), _preferences(false), _constraints(false),
      _probabilistic_effects(false), _rewards(false) {}

Requirements::Requirements(const Requirements &other)
    : _equality(other._equality), _strips(other._strips),
      ...
      _durative_actions(other._durative_actions), _time(other._time),
      _action_costs(other._action_costs), _modules(other._modules),
      ...
      _preferences(other._preferences), _constraints(other._constraints),
      _probabilistic_effects(other._probabilistic_effects),
...
bool Requirements::has_equality() const { return _equality; }
...
bool Requirements::has_strips() const { return _strips; }
...
bool Requirements::has_typing() const { return _typing; }

Requirements &
Requirements::set_negative_preconditions(bool negative_preconditions) {
...
Requirements &
Requirements::set_disjunctive_preconditions(bool disjunctive_preconditions) {
...
Requirements &
Requirements::set_existential_preconditions(bool existential_preconditions) {
...
Requirements &
Requirements::set_universal_preconditions(bool universal_preconditions) {
...
bool Requirements::has_durative_actions() const { return _durative_actions; }
...
bool Requirements::has_object_fluents() const { return _object_fluents; }
...
bool Requirements::has_numeric_fluents() const { return _numeric_fluents; }
...
bool Requirements::has_modules() const { return _modules; }

Requirements &Requirements::set_adl() {
  _strips = _typing = _negative_preconditions = _disjunctive_preconditions =
      _equality = _existential_preconditions = _universal_preconditions =
          _conditional_effects = true;
  return *this;
}

bool Requirements::has_adl() const {
  return _strips && _typing && _negative_preconditions &&
         _disjunctive_preconditions && _equality &&
         _existential_preconditions && _universal_preconditions &&
...
Requirements &
Requirements::set_duration_inequalities(bool duration_inequalities) {
...
Requirements &
Requirements::set_timed_initial_literals(bool timed_initial_literals) {
...
bool Requirements::has_preferences() const { return _preferences; }
...
bool Requirements::has_constraints() const { return _constraints; }

Requirements &
Requirements::set_probabilistic_effects(bool probabilistic_effects) {
  _probabilistic_effects = probabilistic_effects;
  return *this;
}

bool Requirements::has_probabilistic_effects() const {
  return _probabilistic_effects;
}

Requirements &Requirements::set_rewards(bool rewards) {
  _rewards = rewards;
  return *this;
}

bool Requirements::has_rewards() const { return _rewards; }

Requirements &Requirements::set_mdp() {
  _probabilistic_effects = _rewards = true;
  return *this;
}

bool Requirements::has_mdp() const {
  return _probabilistic_effects && _rewards;
}
...
std::ostream &operator<<(std::ostream &o, const Requirements &r) {
  ...
  if (r.has_fluents())
    ...
    if (r.has_numeric_fluents())
      o << " :numeric-fluents";
    if (r.has_object_fluents())
      o << " :object-fluents";
  }
  if (r.has_time())
    o << " :time";
  else {
    if (r.has_numeric_fluents() && !r.has_fluents())
      o << " :numeric-fluents";
    if (r.has_durative_actions())
      o << " :durative-actions";
  }
  if (r.has_action_costs())
    o << " :action-costs";
  else {
    if (r.has_numeric_fluents() && !r.has_fluents() && !r.has_time())
      o << " :numeric-fluents";
  }
  if (r.has_adl())
    o << " :adl";
    ...
    if (r.has_typing())
      o << " :typing";
    if (r.has_negative_preconditions())
      o << " :negative-preconditions";
    if (r.has_disjunctive_preconditions())
      o << " :disjunctive-preconditions";
    if (r.has_equality())
      o << " :equality";
    if (r.has_existential_preconditions())
      o << " :existential-preconditions";
    if (r.has_universal_preconditions())
      o << " :universal-preconditions";
    if (r.has_conditional_effects())
      o << " :conditional-effects";
  }
  if (r.has_quantified_preconditions())
    o << " :quantified-preconditions";
  else {
    if (r.has_existential_preconditions())
      o << " :existential-preconditions";
    if (r.has_universal_preconditions())
  ...
  if (r.has_modules())
    o << " :modules";
  if (r.has_duration_inequalities())
    o << " :duration-inequalities";
  if (r.has_continuous_effects())
    o << " :continuous-effects";
  if (r.has_derived_predicates())
    o << " :derived-predicates";
  if (r.has_timed_initial_literals())
    o << " :timed-initial-literals";
  if (r.has_preferences())
    o << " :preferences";
  ...
  if (r.has_mdp())
    ...
    if (r.has_probabilistic_effects())
      o << " :probabilistic-effects";
    if (r.has_rewards())
      o << " :rewards";
  }
  o << ")" << std::endl;
