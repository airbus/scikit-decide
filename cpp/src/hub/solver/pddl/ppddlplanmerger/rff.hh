 ...
 */
#ifndef SKDECIDE_RFF_HH
#define SKDECIDE_RFF_HH
...
/**
 * @brief RFF (Robust FF): plan-merging solver for PPDDL domains using FF
 * as the inner deterministic planner.
 *
 * Thin wrapper around PPDDLPlanMergerSolver with the inner solver fixed to FF.
 *
...
public:
  ...
  typedef std::function<bool(const RFFSolver &)> CallbackFunctor;
   ...
   */
  RFFSolver(
      const Task &task, double dead_end_cost = 1e9, double rho = 0.1,
      ...
      const CallbackFunctor &callback = [](const RFFSolver &) { return false; },
      bool verbose = false);
  ...
  typename PPDDL::Solver::PolicyMap get_policy() const;
