  ...
  public:
    Implementation(
      ...
      _solver = std::make_unique<
          skdecide::AOStarSolver<PyAOStarDomain<Texecution>, Texecution>>(
          *_domain,
          [this](PyAOStarDomain<Texecution> &d,
                 const typename PyAOStarDomain<Texecution>::State &s) ->
          typename PyAOStarDomain<Texecution>::Predicate {
            try {
          ...
          [this](PyAOStarDomain<Texecution> &d,
                 const typename PyAOStarDomain<Texecution>::State &s) ->
          typename PyAOStarDomain<Texecution>::Value {
            try {
              auto fh = [this](const py::object &dd, const py::object &ss,
                               [[maybe_unused]] const py::object &ii) {
                return _heuristic(dd, ss);
              };
              return typename PyAOStarDomain<Texecution>::Value(
    ...
    virtual py::object get_next_action(const py::object &s) {
      try {
        ...
        Logger::warn(std::string("[AOStar.get_next_action] ") + e.what() +
                     " - returning None");
        return py::none();
      }
    }
    ...
    virtual py::object get_utility(const py::object &s) {
      try {
        ...
        Logger::warn(std::string("[AOStar.get_utility] ") + e.what() +
                     " - returning None");
        return py::none();
      }
    }
