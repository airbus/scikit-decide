 ...
 */
#ifndef SKDECIDE_PY_VI_HH
#define SKDECIDE_PY_VI_HH
...
template <typename Texecution> using PyVIDomain = PythonDomainProxy<Texecution>;
...
private:
  ...
  public:
    ...
    virtual py::set get_converged_states() = 0;
    virtual py::set get_states_updated_in_last_sweep() = 0;
  ...
  public:
    Implementation(
      ...
      check_domain(domain);
      _domain = std::make_unique<PyVIDomain<Texecution>>(domain);
      _solver = std::make_unique<
          skdecide::VISolver<PyVIDomain<Texecution>, Texecution>>(
          *_domain,
          [this](PyVIDomain<Texecution> &d,
                 const typename PyVIDomain<Texecution>::State &s) ->
    ...
    void check_domain(py::object &domain) {
      if (!py::hasattr(domain, "get_applicable_actions")) {
        throw std::invalid_argument(
            "SKDECIDE exception: Value Iteration needs python domain for "
    ...
    virtual py::object get_next_action(const py::object &s) {
      try {
        ...
        Logger::warn(std::string("[VI.get_next_action] ") + e.what() +
                     " - returning None");
        return py::none();
      }
    }
    ...
    virtual py::object get_utility(const py::object &s) {
      try {
        ...
        Logger::warn(std::string("[VI.get_utility] ") + e.what() +
                     " - returning None");
        return py::none();
      }
    }
    ...
    virtual py::set get_converged_states() {
      py::set s;
      auto &&cs = _solver->get_converged_states();
      for (auto &c : cs) {
        s.add(c.pyobj());
      }
      return s;
    ...
    virtual py::set get_states_updated_in_last_sweep() {
      py::set s;
      auto &&us = _solver->get_states_updated_in_last_sweep();
      for (auto &u : us) {
        s.add(u.pyobj());
      }
      return s;
  ...
  private:
    ...
    std::unique_ptr<PyVIDomain<Texecution>> _domain;
    std::unique_ptr<skdecide::VISolver<PyVIDomain<Texecution>, Texecution>>
        _solver;
...
public:
  PyVISolver(
      ...
      const std::function<py::object(const py::object &)> &terminal_value =
          nullptr,
      double discount = 0.999, double epsilon = 0.001,
      std::size_t max_sweeps = 0, bool parallel = false,
      const std::function<py::bool_(const py::object &)> &callback = nullptr,
      bool verbose = false) {

    TemplateInstantiator::select(ExecutionSelector(parallel),
                                 SolverInstantiator(_implementation))
        .instantiate(solver, domain, heuristic, terminal_value, discount,
                     epsilon, max_sweeps, callback, verbose);
  }
  ...
  py::set get_converged_states() {
    return _implementation->get_converged_states();
  }

  py::set get_states_updated_in_last_sweep() {
    return _implementation->get_states_updated_in_last_sweep();
  }
